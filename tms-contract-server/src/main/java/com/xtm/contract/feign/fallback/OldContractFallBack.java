...
@Slf4j
@Component
public class OldContractFallBack implements OldContractFeign {
    @Override
    ...
    @Override
    public Result insertOrUpd(ContractCreUpdReq contractCreUpdReq) {
    ...
    public Result<ContractSignResVo> serviceEcSign(ContractSignVo<?> contractSignVo) {
        log.error("服务合同电子签章服务调用失败");
        return Result.error("服务合同电子签章服务调用失败,请稍后再试");
    }

    @Override
    public Result<ContractSignResVo> energyEcSign(ContractSignVo<?> contractSignVo) {
        log.error("能源合同电子签章服务调用失败");
        return Result.error("能源合同电子签章服务调用失败,请稍后再试");
    }
    ...
    @Override
    public Result<FddElectricSealVo> findFddSealResult(Integer accountType, String operatorId) {
    ...
    @Override
    public Result<List<PartnerFraContractVO>> selectPartnerFraContractList(FrameContractPartnerReq partnerReq) {
        log.error("查询伙伴合同列表失败，请稍后再试");
        return Result.error(ErrorCodeEnum.ERROR.getCode(), "查询伙伴合同列表失败，请稍后再试");
    }
